...
export default function ProfilePage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      if (data.user) {
        ...
        if (dbData) {
          setDbUser(dbData)
          setEditName(dbData.name || data.user.user_metadata?.name || '')
          setEditBio(dbData.bio || data.user.user_metadata?.bio || '')
          setEditAvatar(dbData.avatar || data.user.user_metadata?.avatar_url || '')
          
          // Fetch data associated with this real user
          ...
          setEditAvatar(data.user.user_metadata?.avatar_url || '')
        }
      }
      setChecking(false)
    })
  ...
  async function saveProfile() {
    ...
    // 1. Update Database fields so Feed and Leaderboard sync perfectly
    const { data: updatedDbUser, error: dbError } = await supabase
      .from('users')
      .update({ name: editName, bio: editBio, avatar: editAvatar })
      .eq('id', user.id)
      .select()
      .single()
      
    // 2. Update Auth metadata as fallback
    await supabase.auth.updateUser({
      data: { name: editName, bio: editBio, avatar_url: editAvatar }
    })

    if (!dbError && updatedDbUser) setDbUser(updatedDbUser)
  ...
  const handle    = dbUser?.handle || user?.user_metadata?.handle || userName.toLowerCase().replace(/\s/g, '_')
  const bio       = dbUser?.bio || user?.user_metadata?.bio || 'No bio provided yet.'
  const aura      = dbUser?.aura_points || 0
  const followersCount = dbUser?.followers || 0
  const followingCount = dbUser?.following || 0
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-white selection:bg-[#f5c140]/30">
      ...
      <TopNavbar 
        avatarUrl={avatarUrl} userName={userName} userInitial={userName[0]?.toUpperCase()} 
      ...
      <div className="mx-auto max-w-7xl px-4 pt-[57px]">
        <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] xl:grid-cols-[240px_1fr_300px] min-h-screen relative">
          <LeftSidebar activeNav="profile" avatarUrl={avatarUrl} userName={userName} userInitial={userName[0]?.toUpperCase()} signOut={() => supabase.auth.signOut()} />
          ...
          <main className="border-x border-white/[0.055] min-h-screen pb-20 md:pb-0 bg-black/20">
            ...
            <div className="relative px-6 pb-6">
              <div className="flex justify-between items-end -mt-20 mb-6">
                ...
                >
                  {/* Spinning Gradient Halo on Hover */}
              ...
              >
                ...
                <p className="text-[15px] text-white/40 font-medium">@{handle}</p>
              </motion.div>

              <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px] text-white/50 mb-6 font-medium">
                {dbUser?.role?.[0] && <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/5"><Briefcase size={14} className="text-white/30" /> {dbUser.role[0]}</span>}
                {dbUser?.college && <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/5"><GraduationCap size={14} className="text-white/30" /> {dbUser.college}</span>}
                {dbUser?.created_at && <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/5"><Calendar size={14} className="text-white/30" /> Joined {new Date(dbUser.created_at).toLocaleDateString(undefined, { month: 'short', year:'numeric' })}</span>}
              </div>

              <p className="text-[15px] leading-relaxed text-white/70 max-w-2xl mb-8 font-medium italic">&quot;{bio}&quot;</p>

              {/* Tags from user stack */}
              {dbUser?.stack && dbUser.stack.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-8">
                  {dbUser.stack.map((t: string) => (
                ...
                >
                  {activeTab === 'Posts' && (
                    <div className="divide-y divide-white/[0.04]">
                      {posts.length > 0 ? posts.map((p, i) => <PostCard key={p.id} post={p} index={i} currentUser={user} />) : <div className="p-8 text-center text-white/30 text-sm">No posts yet.</div>}
                    </div>
                  )}
                    ...
                    <div className="px-6 py-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                      {projects.length > 0 ? projects.map((project, i) => (
                        ...
                        <div className="col-span-full py-12 flex flex-col items-center justify-center text-center">
                          ...
                          <p className="text-[13px] text-white/40">Share your engineering masterpieces with the world.</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {(activeTab === 'Achievements' || activeTab === 'Activity') && (
                    <div className="p-12 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-[13px] text-white/40">Keep building and participating to grow your {activeTab.toLowerCase()}.</p>
                    </div>
                  )}
                </motion.div>
              </AnimatePresence>
      ...
      <MobileBottomNav activeNav="profile" />
        ...
        {isEditing && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
            <motion.div 
              ...
              className="w-full max-w-md rounded-3xl bg-[#111] border border-white/10 shadow-2xl overflow-hidden"
            >
              <div className="px-5 py-4 border-b border-white/10 flex justify-between items-center">
                <h2 className="text-[16px] font-bold">Edit Profile</h2>
                <button onClick={() => setIsEditing(false)} className="text-white/40 hover:text-white"><X size={18} /></button>
              </div>
              <div className="p-5 space-y-4">
                <div>
                  <label className="block text-[11px] font-bold text-white/40 uppercase mb-1.5">Display Name</label>
                  <input value={editName} onChange={e => setEditName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-[14px] text-white outline-none focus:border-[#f5c140]/50" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-white/40 uppercase mb-1.5">Bio</label>
                  <textarea value={editBio} onChange={e => setEditBio(e.target.value)} rows={3} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-[14px] text-white outline-none focus:border-[#f5c140]/50 resize-none" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-white/40 uppercase mb-1.5">Avatar URL (Optional)</label>
                  <input value={editAvatar} onChange={e => setEditAvatar(e.target.value)} placeholder="https://..." className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-[14px] text-white outline-none focus:border-[#f5c140]/50 text-white/60" />
                </div>
              </div>
              <div className="px-5 py-4 bg-white/5 border-t border-white/10 flex justify-end gap-3">
                <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-[13px] font-bold text-white/60 hover:text-white">Cancel</button>
                <button 
                  onClick={saveProfile}
                  disabled={saving}
                  className="px-5 py-2 rounded-full bg-[#f5c140] text-[13px] font-bold text-black disabled:opacity-50 hover:bg-[#fde68a] transition-colors"
                >
      ...
      <AnimatePresence>
        {showFollowers && user && (
          <FollowsModal userId={user.id} type="followers" onClose={() => setShowFollowers(false)} currentUser={user} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showFollowing && user && (
          <FollowsModal userId={user.id} type="following" onClose={() => setShowFollowing(false)} currentUser={user} />
        )}
      </AnimatePresence>
    </div>
  )
